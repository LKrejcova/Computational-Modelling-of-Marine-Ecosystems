...
# growth.lim <- function(P, params){
#   I = lightloss(P, params)
#   growth.L <- I / (param.HI + I)
#   growth.N <- N / (param.HN + N)
#   return(rbind(growth.L, growth.N))
# }

growth.lim.L <- function(P, params){
  growth.L <- lightloss(P, params) / (param.HI + lightloss(P, params))
  return(growth.L)
}

growth.lim.N <- function(N, params){
  growth.N <- N / (param.HN + N)
  return(growth.N)
}
growth <- function(P, params){
  I = lightloss(P, params)
  growth <- param.gmax * pmin(growth.lim.L(P,params), growth.lim.N(P, params)) - param.m
  return(growth)
}
...
derivative = function(t, Y, params) {
  ...
  for (i in 1:n){
    dphi_dt[i] <- g[i]*phi[i] - (J[i+1]- J[i])/param.deltaZ
    dN_dt[i] <- -param.yield*g[i]*phi[i] - (JN[i+1]- JN[i])/param.deltaZ
  }
...
# growth.lim.N <- N.out[nrow(N.out),] / (param.HN + nrow(N.out))


par(mar=c(5, 4, 4, 8), xpd=TRUE) # adds space next to the plot so we can put a legend there
plot(x = growth.lim.L(phi.out,params)[1:n], y = n:1, type = "l", xlim = c(0, max(growth.lim.N(N.out, params)[1:n])),
     main = "Limitation by light or nutrients", ylab = "Distance from seabed [m]",
     xlab = "Growth limitation factor", lwd = 3)
lines(x = growth.lim.N(N.out[n,], params), y = n:1, type = "l", lwd = 3, lty = 2)
